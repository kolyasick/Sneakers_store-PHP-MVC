<h1 class="text-3xl mb-5 font-bold">Создание товара</h1>
<?php if(!empty($error)): ?>
    <div class="bg-red p-3 border border-red-300"><?= $error?></div>
    <?php endif; ?> 
<form action="/myProject2.loc/admin/product/add" method="post" class="w-2/4">
...
<br>
    ...
    <input class="border border-gray-400 border rounded-md w-full p-1" type="text" name="title" value="<?= $_POST['title'] ?? '' ?>" size="50"><br>
    <br>
    <label class="font-semibold" for="name">Предназначение товара</label><br>
    <input class="border border-gray-400 border rounded-md w-full p-1" type="text" name="content" value="<?= $_POST['content'] ?? '' ?>" size="50"><br>
    <br>
    <label class="font-semibold" for="name">Цена товара</label><br>
    <input class="border border-gray-400 border rounded-md w-full p-1" type="number" name="price" value="<?= $_POST['price'] ?? '' ?>" size="50"><br>
    <br>
    <label class="font-semibold" for="name">Старая цена товара</label><br>
    <input class="border border-gray-400 border rounded-md w-full p-1" type="number" name="oldPrice" value="<?= $_POST['oldPrice'] ?? '' ?>" size="50"><br>
    <br>
    <label class="font-semibold" for="name">Картинка товара(url)</label><br>
    <input class="border border-gray-400 border rounded-md w-full p-1" type="text" name="img" value="<?= $_POST['img'] ?? '' ?>" size="50"><br>
    <br>
