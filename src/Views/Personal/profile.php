<div class="text-center w-full mx-auto">
    <ul class="flex gap-1 items-center justify-center">
        ...
        <li>
            <a class="text-gray-400" href="/myProject2.loc/personal">Профиль</a>
        </li>
    </ul>
</div>
...
<div class="w-1/3 border border-slate-400 rounded-xl flex items-center justify-between py-4 flex-col">
  <div class="p-5 h-auto w-3/5 flex justify-center">
    <img width="150" src="/myProject2.loc/public/user.png" alt="">
  </div>
  ...
  <div class="text-lg">
    ...
    <a class="border border-gray-400 border rounded-md px-3 py-1 cursor-pointer transition hover:bg-gray-300 w-full" href="/myProject2.loc/personal/<?= $user->getId() ?>/orders">Мои заказы</a>
  </div>
</div>
<div class="flex">
<form class="border border-gray-400 border rounded-xl p-5 mb-10 inline-block h-full" action="/myProject2.loc/personal/<?= $user->getId() ?>/edit" method="POST">
...
<?php if(!empty($message)): ?>
    <div class="bg-green-600 text-white px-4 py-4  border border-green-300 inline relative h-1/8 self-end ms-2 rounded-xl">
        <p><?= $message?></p>
        <a class="absolute -top-2 -right-2 text-xl" href="/myProject2.loc/personal">
            <img width="40" src="/myProject2.loc/public/krest.svg" alt="">
        </a>
    </div>
<?php endif ?> 
<?php if(!empty($error)): ?>
